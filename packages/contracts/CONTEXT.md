...
│   │   ├── conversations.ts    S6 request/response DTOs: OpenConversation, SendTextMessage, ConversationSummary, MessageSummary, ConversationListingCard, cursor pagination for messages. Text-only MLP; no rich-chat fields.
...
S7 target and actor summaries are stable DTOs. `targetSummary` uses an `available` boolean plus `label`, `targetType`, and `targetId`; unavailable targets use `available = false` and label `"Unavailable target"` while preserving ids. Report detail can add detail-only target fields, but report list rows cannot. Audit `actorSummary` distinguishes deleted authenticated admins from operator scripts; `ADMIN_BOOTSTRAP_PROMOTE` with `actorId = null` is operator provenance, not a deleted admin.
...
OpenAPI generation is part of S7 contract ownership. The contracts child issue must add/administer the Zod schemas and reproducible OpenAPI diff before API/admin route consumers depend on them.
