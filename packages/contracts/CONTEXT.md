...
Most per-schema files (e.g., `schemas/listings.ts`, `schemas/conversations.ts`) are minimal placeholders today — they get populated as the owning sprint adds API surface. Currently only `auth.ts` (S2), `identity.ts` (S2), and `catalog.ts` (partial — `BrandSummarySchema` only) have real content.
