...
The package export points at built CommonJS output in `dist/` so runtime consumers do not execute raw TypeScript. `dist/` is ignored and regenerated by `pnpm --filter @auto-tm/contracts build`. See [ADR-0016](../../docs/adr/0016-typescript-runtime-boundaries.md).
