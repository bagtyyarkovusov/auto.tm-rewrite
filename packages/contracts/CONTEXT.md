...
Most per-schema files (e.g., `schemas/listings.ts`, `schemas/conversations.ts`) are minimal placeholders today — they get populated as the owning sprint adds API surface. Currently `auth.ts` (S2), `identity.ts` (S2), and `catalog.ts` (S3: full read DTOs + list response schemas for the 7 shipped catalog entities, plus Brand/Model admin write DTOs) have real content. `EngineType`, `Transmission`, and `DriveType` catalog DTOs have not shipped yet.
