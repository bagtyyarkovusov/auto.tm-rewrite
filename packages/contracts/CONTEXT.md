...
│   │   └── admin.ts            S7 admin op schemas: ContentReport create/list/detail DTOs, moderation command request/response DTOs (dismiss/ban/unban/suspend/unsuspend), audit list/detail DTOs, report reason enum, content-report status enum, audit action constants, canonical details.reason constants, targetSummary/actorSummary DTOs
...
Most per-schema files (e.g., `schemas/listings.ts`, `schemas/conversations.ts`) are minimal placeholders today — they get populated as the owning sprint adds API surface. Currently `auth.ts` (S2 + S7), `identity.ts` (S2), `catalog.ts` (S3: full read DTOs + list response schemas for the 7 shipped catalog entities, plus Brand/Model admin write DTOs; EngineType/Transmission/DriveType DTOs also present), `listings.ts` (S4: full ListingSummary with `coverMediaKey`, ListingDetail with `acceptsExchange`/`installmentAvailable`/`contactPhone`/`allowCalls`/`allowChat`, ListingMedia with `variants` for `thumbnail`/`list`/`detail`/`fullscreen`, ListingDraft, request/response DTOs, cursor helpers, error codes; S8: `favoriteCount`, optional `isFavorited`, add/remove favorite responses, and `MyFavoritesResponseSchema`; **S9a: `sellerTrust.phoneVerified` on ListingSummary and ListingDetail; `ConditionDisclosureSchema` and optional `conditionDisclosure` on ListingDetail, ListingDraftPayload, and EditListingRequest; `VinHistorySchema` and optional `vinHistory` on ListingDetail**; **#216: `ListingCountQuerySchema` (same filter fields as `FeedQuerySchema` without `cursor`/`limit`) and `ListingCountResponseSchema` (`{ totalMatching: non-negative integer }`)**; **#217: `modelIds` added to `ListingFilterFieldsSchema`/`FeedQuerySchema`/`ListingCountQuerySchema` as an array of up to 50 UUIDs; `ListingFilterSchema` rejects simultaneous `modelId`+`modelIds` and requires `brandId` when `modelIds` is non-empty; `ListingModelCountQuerySchema` (filter fields minus `modelId`/`modelIds`, with required `brandId`), `ListingModelCountItemSchema` (`{ modelId, totalMatching }`), and `ListingModelCountResponseSchema` (`{ items: ListingModelCountItem[] }`)**), `uploads.ts` (S4: presign request/response), `exchange-rates.ts` (S4: FX rate DTOs), `conversations.ts` (S6: text-only per-listing thread DTOs — open, list conversations, list messages, send text message; `ConversationListingCard` for mobile listing summary in thread header; cursor pagination for messages), and `admin.ts` (S7: ContentReport DTOs, moderation command request/response DTOs, audit DTOs, canonical constants) have real content.
...
S10 rich-chat contract additions (schema-only slice, no behavior):
- `schemas/conversations.ts` gains `MessageKindSchema`, `ImageMessageMetadataSchema`, `PostRefMessageMetadataSchema`, `MessageMetadataSchema`, and a richer `MessageSummarySchema` with `kind`, optional `text`, optional `metadata`, `deletedAt`, and `clientMessageId`. Existing S6 text DTOs (`SendTextMessageRequestSchema`/`SendTextMessageResponseSchema`) remain unchanged for backward compatibility.
- New request/response/event DTOs in `schemas/conversations.ts`: `SendMessageRequestSchema` (text/image/post_ref discriminated union), `SendMessageResponseSchema`, `UpdateWatermarkRequestSchema`/`UpdateWatermarkResponseSchema`, `MuteConversationRequestSchema`/`MuteConversationResponseSchema`, `DeleteMessageResponseSchema`, plus socket payloads `ChatMessageEventSchema`, `MessageDeletedEventSchema`, `TypingEventSchema`, and `WatermarkEventSchema`.
