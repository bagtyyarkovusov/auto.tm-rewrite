...
- `src/queues/email-code.processor.ts` — handles `sign-in-code` jobs enqueued by the API identity adapter on the `email-code` queue. The request UUID is the BullMQ job id and Resend idempotency key; the producer gives retryable failures 3 exponential attempts. The processor validates `{ to, code, locale, purpose }` and runs `SendSignInCodeEmail`. Provider rejection, daily cap, invalid payload, missing job id, or unknown name throw `UnrecoverableError`; a retryable provider failure throws a plain error. Codes are never logged; recipient logs are masked.
...
- `domain/types.ts` — `EMAIL_DRIVER`, `EMAIL_SEND_FAILURE`, `SEND_SIGN_IN_CODE_OUTCOME`, `EMAIL_DAILY_CAP_REACHED_ALERT`, `EMAIL_LOCALE`, `SIGN_IN_CODE_PURPOSE` (mirroring the contracts enums), `EMAIL_SIGN_IN_CODE_EXPIRY_MINUTES` (10) and `DEFAULT_EMAIL_DAILY_CAP` (80).
