...
- `adapters/apns/ApnsSender.ts` — `ParseApnsSender` wraps a single injected node-apn send function; `createApnsSendFn` builds a token-based `apn.Provider` and an alert `apn.Notification` topic-scoped to `APNS_BUNDLE_ID`.
...
- `adapters/credentials.ts` — `normalizePrivateKey` unquotes and expands escaped (`\n`, `\r\n`) PEM environment values and rejects non-PEM input; `readFcmCredentials` / `readApnsCredentials` assemble the provider credential sets. Errors name the variable and never echo key material.
...
- Push credentials (required together when `PUSH_TRANSPORT=fcm-apns`): `FCM_PROJECT_ID`, `FCM_CLIENT_EMAIL`, `FCM_PRIVATE_KEY`, `APNS_KEY_ID`, `APNS_TEAM_ID`, `APNS_BUNDLE_ID`, `APNS_PRIVATE_KEY`. `FCM_PRIVATE_KEY` and `APNS_PRIVATE_KEY` may be shell-quoted with escaped `\n` newlines; both are parsed at boot so a malformed secret crashes startup instead of silently failing every send
