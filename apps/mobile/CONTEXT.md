...
Listing components live at `src/listings/components/` (`ListingDetail.tsx`, `PhotoGallery.tsx`, `PriceDisplay.tsx`, `SellerBlock.tsx`, `ContactCtaBar.tsx`, `OwnerActions.tsx`, `OwnerListingCard.tsx`, `DraftCard.tsx`). `PriceDisplay` supports asymmetric owner mode (TMT + original currency). `OwnerActions` renders status-aware controls with `AlertDialog` confirmations. `OwnerListingCard` adapts the feed card visual language for owner management with status badge, Open, and Edit actions. `DraftCard` shows draft identity, progress, photo count, Resume, and destructive Discard with `AlertDialog` confirmation. `ContactCtaBar` enables Message for eligible non-owner active listings with `allowChat=true`; anonymous taps store auth intent and route through OTP; authenticated taps call `useOpenConversation` and navigate to `/conversations/[id]`. Favorite button toggles optimistically via `useFavoriteListing` / `useUnfavoriteListing`, rolls back on mutation error, and receives initial `isFavorited` state from the parent detail screen.
