...
Conversation hooks live at `src/api/conversations/*` (`useConversations`, `useConversationMessages`, `useOpenConversation`, `useSendTextMessage`, `useUpdateWatermark`, `useDeleteMessage`). `useConversations` and `useConversationMessages` are `useInfiniteQuery` hooks with cursor pagination. `useOpenConversation` invalidates the conversation list and detail on success. `useSendTextMessage` invalidates messages, conversation list, and conversation detail on success. `useUpdateWatermark` posts to `POST /api/v1/conversations/:id/watermark` and is used as an HTTP fallback when the socket watermark emit fails. `useDeleteMessage` calls `DELETE /conversations/:id/messages/:messageId`, parses `DeleteMessageResponseSchema`, and invalidates messages, conversation list, and conversation detail on success; it is used as an HTTP fallback when the socket delete fails.
Realtime chat hooks live at `src/conversations/socket/*` (`ConversationSocket.ts`, `useConversationSocket.ts`). `ConversationSocket` wraps `socket.io-client`, loads the JWT access token from `expo-secure-store`, connects to `EXPO_PUBLIC_WS_URL`, joins `conversation:{conversationId}` rooms, emits `message:send` with `clientMessageId`, emits `message:delivered`, `message:read`, and `conversation:read` for watermark updates, emits `message:delete` for own-message soft delete, emits `typing:start`/`typing:stop` for ephemeral typing indicators, and listens for `message:new`, `watermark`, `message:deleted`, `typing:peer`, and `presence`. `useConversationSocket` manages the lifecycle for a single conversation, patches the TanStack Query messages cache on `message:new` (deduped by `id` and `clientMessageId`) and on `message:deleted` (redacts `text`/`metadata` and sets `deletedAt`), invalidates the messages query on socket reconnect so HTTP refetches any missed events, auto-marks incoming peer messages as delivered once, marks the conversation read when the thread opens, invalidates the conversation list/detail on `watermark` events, exposes `sendTextMessage` and `deleteMessage`, and manages local typing state: `signalTyping()` sends `typing:start` once and debounces `typing:stop` for 3 s; `stopTyping()` sends `typing:stop` immediately; incoming `typing:peer` events drive `peerTyping` with a 5 s client-side timeout; incoming `presence` events drive `peerPresence` (`online` + `lastSeenAt`).
