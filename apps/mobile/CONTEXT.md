...
Realtime chat hooks live at `src/conversations/socket/*` (`ConversationSocket.ts`, `useConversationSocket.ts`). `ConversationSocket` wraps `socket.io-client`, loads the JWT access token from `expo-secure-store`, connects to `EXPO_PUBLIC_WS_URL`, joins `conversation:{conversationId}` rooms, emits `message:send` with `clientMessageId`, and listens for `message:new`. `useConversationSocket` manages the lifecycle for a single conversation, patches the TanStack Query messages cache on `message:new` (deduped by `id` and `clientMessageId`), and invalidates the messages query on socket reconnect so HTTP refetches any missed events.
