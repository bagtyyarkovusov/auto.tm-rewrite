...
- **`@tanstack/react-query@^5.100.10`** for server cache + mutations, layered on a small custom `apiClient` wrapper at `src/api/client.ts`. See [ADR-0015](../../docs/adr/0015-mobile-data-fetching.md) and [`docs/agents/mobile-data-fetching.md`](../../docs/agents/mobile-data-fetching.md). Query keys factory at `src/api/queryKeys.ts` (covers `catalog.*`, `listings.*`, `uploads.*`, `exchangeRates.*`, `conversations.*`). The wrapper uses `AbortController` with a 30-second default request timeout (15 seconds for refresh); timeouts throw `ApiError("NETWORK_ERROR", 0, "Request timed out")`.
...
/(tabs)/
  ...
  chat                Conversation list                   — stub (conversation detail ships with #172)
...
/conversations/
  ...
  [id]                Conversation detail (text thread)    — #172; compact listing card at top (tap to open listing detail), message list with optimistic pending/confirmed/failed states and retry, composer with 1000-char limit; uses `useConversationMessages` (infinite query) and `useSendTextMessage`
...
Conversation UI components live at `src/conversations/components/` (`ConversationListingCard.tsx`, `MessageBubble.tsx`, `MessageComposer.tsx`, `MessageList.tsx`). `ConversationListingCard` renders a compact tap-to-navigate listing card (cover image, title, price, status). `MessageBubble` supports `confirmed` / `pending` / `failed` status with retry affordance. `MessageComposer` is a `KeyboardAvoidingView` wrapper around a multiline `TextInput` (1000-char max, blank rejection) and a send button. `MessageList` is an inverted `FlatList` of `MessageBubble`s.
Listing components live at `src/listings/components/` (`ListingDetail.tsx`, `PhotoGallery.tsx`, `PriceDisplay.tsx`, `SellerBlock.tsx`, `ContactCtaBar.tsx`, `OwnerActions.tsx`, `OwnerListingCard.tsx`, `DraftCard.tsx`). `PriceDisplay` supports asymmetric owner mode (TMT + original currency). `OwnerActions` renders status-aware controls with `AlertDialog` confirmations. `OwnerListingCard` adapts the feed card visual language for owner management with status badge, Open, and Edit actions. `DraftCard` shows draft identity, progress, photo count, Resume, and destructive Discard with `AlertDialog` confirmation. `ContactCtaBar` enables Message for eligible non-owner active listings with `allowChat=true`; anonymous taps store auth intent and route through OTP; authenticated taps call `useOpenConversation` and navigate to `/conversations/[id]`.
...
- **S6 (Contact seller)** — conversation data hooks shipped (`src/api/conversations/*`). Conversation list/detail routes and visible UI remain planned; no `socket.io-client` dependency required for MLP
	- **S7 (Minimal admin + moderation)** — report-entry UI as needed by beta safety. Show listing report affordances only for active non-owner listing detail; hide them for owner, sold, archived, banned, draft, or unavailable listing states. User report affordances require a visible public target and are hidden for the viewer's own profile; mobile does not special-case visible admin users or expose staff role during report creation. There is no global report action, no report-from-message entry point unless S6 is explicitly reshaped, and no public report management surface. Anonymous report taps use the existing auth-on-action pattern: resume the report sheet after login if cheap in S7, otherwise return to target detail and require one more Report tap. Report form submits canonical reason enum values plus optional plain-text details; client UI may trim and enforce required `other` details and the 1000-char cap before submit, but API validation/storage owns the trim, blank-after-trim `other` rejection, and final cap. S7 has no client-visible report-specific quota or remaining-count UI; report submission handles the standard global `RATE_LIMITED` error only. Suspended reporter submit shows generic account-restricted UI. New report and duplicate pending reuse both show the same generic success copy: "Thanks, we received your report." Mobile never displays report id, queue status, duplicate/reused state, reviewer state, other reporters' counts, or target staff role. If a target becomes unavailable or non-reportable between load and submit, mobile shows generic target-unavailable copy: "This item is no longer available to report." Reporting is submit-only in S7: no report history screen, status tracking, edit flow, retract flow, appeals/support flow, resolution notification, or admin-reason exposure. Reported listing owners/users see no report metadata in mobile surfaces: no report count, reason/details, reporter identity, status, or admin notes; moderated targets show only generic banned/suspended state. Pending reports do not block owner archive/delete or account deletion UI in S7.
