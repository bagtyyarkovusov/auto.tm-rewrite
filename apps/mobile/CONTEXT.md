...
Conversation UI components live at `src/conversations/components/` (`ConversationList.tsx`, `ConversationListItem.tsx`, `ConversationListingCard.tsx`, `MessageBubble.tsx`, `MessageComposer.tsx`, `MessageList.tsx`). `ConversationList` renders the authenticated conversation list with loading skeleton, empty state, error/retry, pull-to-refresh via `RefreshControl`, and infinite scroll. `ConversationListItem` is a compact row (listing thumbnail, title/price, last message preview, updated time, role badge, listing status when non-active) that navigates to `/conversations/[id]` with listing card params. `ConversationListingCard` renders a compact tap-to-navigate listing card (cover image, title, price, status). `MessageBubble` supports `confirmed` / `pending` / `failed` status with retry affordance. `MessageComposer` is a `KeyboardAvoidingView` wrapper around a multiline `TextInput` (1000-char max, blank rejection) and a send button. `MessageList` is an inverted `FlatList` of `MessageBubble`s.
