...
Conversation hooks live at `src/api/conversations/*` (`useConversations`, `useConversationMessages`, `useOpenConversation`, `useSendTextMessage`, `useDeleteMessage`). `useConversations` and `useConversationMessages` are `useInfiniteQuery` hooks with cursor pagination. `useOpenConversation` invalidates the conversation list and detail on success. `useSendTextMessage` invalidates messages, conversation list, and conversation detail on success. `useDeleteMessage` calls `DELETE /conversations/:id/messages/:messageId`, parses `DeleteMessageResponseSchema`, and invalidates messages, conversation list, and conversation detail on success; it is used as an HTTP fallback when the socket delete fails.
Realtime chat hooks live at `src/conversations/socket/*` (`ConversationSocket.ts`, `useConversationSocket.ts`). `ConversationSocket` wraps `socket.io-client`, loads the JWT access token from `expo-secure-store`, connects to `EXPO_PUBLIC_WS_URL`, joins `conversation:{conversationId}` rooms, emits `message:send` with `clientMessageId`, emits `message:delete` for own-message soft delete, and listens for `message:new` and `message:deleted`. `useConversationSocket` manages the lifecycle for a single conversation, patches the TanStack Query messages cache on `message:new` (deduped by `id` and `clientMessageId`) and on `message:deleted` (redacts `text`/`metadata` and sets `deletedAt`), exposes `sendTextMessage` and `deleteMessage`, and invalidates the messages query on socket reconnect so HTTP refetches any missed events.
