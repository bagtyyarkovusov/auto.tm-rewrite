...
Admin hooks live at `src/api/admin/*` (`useCreateReport`). `useCreateReport` is a `useMutation` hook that submits reports via `POST /{targetType}s/{targetId}/report` and parses the response through `AdminSchemas.CreateReportResponseSchema`.
