...
Realtime chat hooks live at `src/conversations/socket/*` (`ConversationSocket.ts`, `useConversationSocket.ts`). `ConversationSocket` wraps `socket.io-client`, loads the JWT access token from `expo-secure-store`, connects to `EXPO_PUBLIC_WS_URL`, joins `conversation:{conversationId}` rooms, emits `message:send` with `clientMessageId`, emits `message:delivered`, `message:read`, and `conversation:read` for watermark updates, emits `message:delete` for own-message soft delete, and listens for `message:new`, `watermark`, and `message:deleted`. `useConversationSocket` manages the lifecycle for a single conversation, patches the TanStack Query messages cache on `message:new` (deduped by `id` and `clientMessageId`) and on `message:deleted` (redacts `text`/`metadata` and sets `deletedAt`), invalidates the messages query on socket reconnect so HTTP refetches any missed events, auto-marks incoming peer messages as delivered once, marks the conversation read when the thread opens, invalidates the conversation list/detail on `watermark` events, and exposes `sendTextMessage` and `deleteMessage`.
