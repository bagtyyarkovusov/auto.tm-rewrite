...
- `apps/mobile/src/listings/`
  ...
  - `components/` — shared listing display components (used by feed + detail + management)
    ...
    - `ContactCtaBar.tsx` — sticky bottom CTA bar, rendered only for non-owners on Listings open for contact (the screen omits it entirely for sold/archived Listings): Call (`tel:` via `expo-linking`, disabled when sold/no-phone/allowCalls=false), Message (enabled for eligible non-owner active listings with `allowChat=true` via #172; anonymous tap stores auth intent and routes to OTP, authenticated tap calls `useOpenConversation` and navigates to `/conversations/[id]`), Share (`react-native` `Share`) using the public web URL `https://auto.tm/listings/{id}`, and Favorite (wired with optimistic toggle via `useFavoriteListing`/`useUnfavoriteListing`). The optimistic favorite state syncs back to the `isFavorited` prop when detail refetches, and CTA labels are capped at one line.
    ...
    - `InspectionInterestCta.tsx` — S9a fake-door CTA rendered inside `ListingDetail.tsx` for active listings; opens an RNR `Sheet` with honest coming-soon copy, optional willingness-to-pay TMT input, auth-on-action for anonymous users, and success/error states; controlled by `ListingDetailScreen` so post-OTP replay and post-publish redirect can reopen it
