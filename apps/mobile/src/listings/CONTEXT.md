...
- `apps/mobile/src/listings/`
  ...
  - `search/` — feed filter sheet + filter state hook
    - `useListingFilters.ts` — hook managing `draft` (in-progress edits), `active` (committed filters), `setField`, `apply`, `reset`, `count`, and `isValid`. Filter type inferred from `@auto-tm/contracts` `ListingFilterSchema`. Apply commits draft → active; reset clears both. `isValid` is `false` when `yearMin > yearMax` or when `modelIds` is non-empty without a `brandId`. Arrays (e.g. `modelIds`) are treated as active only when non-empty.
    - `useListingFilters.spec.ts` — unit tests for apply/reset/count transitions, draft/active isolation, and `isValid` logic.
    - `FilterSheet.tsx` — RNR `Sheet` shell hosting `ConditionFilterControl`, `CityFilterControl`, `BrandModelFilterControl`, `YearRangeFilterControl`, and `PriceRangeFilterControl` in the Kolesa funnel order: Condition → City → Make/Model → Year → Price. Apply closes the sheet and is disabled when `isValid` is `false` or any control reports invalid (price-range min > max); Reset clears all filters. Active-filter count is surfaced on the Search tab's prominent Filter/Search entry via a `Badge`. Sheet height is computed from `useWindowDimensions` (`Math.min(screenHeight * 0.85, screenHeight - 80)`) so it never overflows on small screens. Shows inline `t("checkFilterValues")` error when Apply is disabled. **Live result count:** `FilterSheet` consumes `useListingCount({ filters: draft })` to show the estimated number of matching listings on the Apply button; the count is debounced 300 ms, fetched anonymously via `GET /api/v1/listings/count`, and falls back to the static `t("apply")` label while loading, on error, or when the draft is invalid. Apply button label is capped at one line to keep the fixed footer height stable.
    - `BrandModelFilterControl.tsx` — Brand→Model filter control: brand `PickerRow` + `CatalogPickerSheet`, and a multi-select model sheet (`ModelMultiSelectSheet`). Reuses `useBrands(locale)`, `useModels(brandId, locale)`, and `useListingModelCounts({ filters })` with locale derived from `i18n.language`. Model row disabled until brand selected; selecting a brand clears any previously selected `modelIds`. Models are shown in two groups: "Popular models" (sorted by feed-eligible count descending, then name) and "All models" (alphabetical), with search and checkbox multi-select; a sticky footer shows the selected count and a "Select N models" confirm button. Writes `brandId`/`modelIds` to filter draft via `setField`.
  ...
  - `detail/` — buyer listing detail helpers
    - `useCatalogMaps.ts` — resolves catalog IDs (brand, model, generation, color, bodyType, transmission, driveType, engineType, region, city) to display names using existing public catalog hooks; returns `undefined` while catalog data is loading (callers such as the detail title fall back to the raw ID)
    - `buildVariantUrl.ts` — constructs `expo-image` URLs from MinIO keys and variant names (`detail`, `fullscreen`, etc.)
    - `closedListing.ts` — pure closed-for-contact rules: `isClosedForContact(status)` (true for `sold` and `archived`), `closedListingBannerKey(status)` (`sold` → "Sold", `archived` → "Removed from sale"), and `similarListingsHref(listing)` (Home feed route with `brandId` + `modelId` params, used until a Results screen exists)
  - `components/` — shared listing display components (used by feed + detail + management)
    - `PhotoGallery.tsx` — horizontal paging gallery (`FlatList` + `expo-image` `detail` variant) with dot indicator; tap opens fullscreen modal (`fullscreen` variant); optional `banner` prop draws a dark status strip across the bottom of the photo (also on the no-photo fallback) and lifts the page dots above it; uses `useWindowDimensions` so rotation updates layout; `statusBarTranslucent` on fullscreen modal; close button and page indicator use explicit top/bottom padding instead of safe-area utilities; no-media fallback renders "No photos"
    - `PriceDisplay.tsx` — public/buyer TMT-only price (optional `muted` greys it for closed Listings) + owner asymmetric mode (TMT primary + original currency secondary for USD/AED) + conditional seller-term badges (`Exchange possible`, `Installment possible`). Prices format with `i18n.language` so separators match the active UI locale and are capped at one line to prevent overflow.
    - `SellerBlock.tsx` — private seller label + location context (region/city/locationText) + contact phone when calls enabled + neutral `verifiedPhone` badge when `phoneVerified` is true. Verified badge and avatar are `shrink-0`; text containers use `min-w-0 flex-1` and `numberOfLines` so long names/locations wrap instead of overflowing the card width.
    - `SellerBlock.spec.tsx` — source tests for the verified-phone badge
    - `ContactCtaBar.tsx` — sticky bottom CTA bar, rendered only for non-owners on Listings open for contact (the screen omits it entirely for sold/archived Listings): Call (`tel:` via `expo-linking`, disabled when sold/no-phone/allowCalls=false), Message (enabled for eligible non-owner active listings with `allowChat=true` via #172; a signed-out tap calls `requireSignIn` with `returnTo` set to this Listing and a `{ kind: "message", listingId }` pending action, so sign-in returns here and then opens the Conversation; authenticated tap calls `useOpenConversation` and navigates to `/conversations/[id]`), Share (`react-native` `Share`) using the public web URL `https://auto.tm/listings/{id}`, and Favorite (wired with optimistic toggle via `useFavoriteListing`/`useUnfavoriteListing`; a signed-out tap parks a `{ kind: "favorite", listingId }` pending action and the Favorite is added on return). Both replays run through `useReplayAuthAction` on this same mounted component, so Back from the Listing still returns to Results with its position (#365). The optimistic favorite state syncs back to the `isFavorited` prop when detail refetches, and CTA labels are capped at one line.
    - `ListingDetail.tsx` — full detail composition: title (year + brand + model + generation), sold badge, price block, spec grid (year, condition, mileage, transmission, drive, engine, power, color, body type, VIN — conditional on presence), description, **S9a structured condition disclosure**, **S9a VIN history section with decoded fields and honest empty state**, seller block, and **S9a inspection-interest fake-door CTA**; branches between buyer (`SellerBlock`) and owner (`OwnerActions`) presentation via `isOwner` prop; passes `listing.sellerTrust?.phoneVerified` to `SellerBlock`. **Sold and archived Listings are closed for contact for buyers:** the photo carries a "Sold" / "Removed from sale" banner, the price is greyed, the title Sold badge gives way to the banner, `SellerBlock` gets no phone and no verified badge, Report and the inspection-interest CTA stay active-only, and a "See other {brand} {model}" button (shown once catalog names resolve; the screen passes `onSeeSimilar`) opens the Home feed filtered by that brand + model. The screen hides the whole `ContactCtaBar` (Call, Message, Share, ♡). The owner view of a sold/archived Listing is unchanged. Deleted Listings, and banned Listings for non-owners, are 404 from the API and render the unavailable state. Title, spec values, and trust-info link use `min-w-0`/`numberOfLines` so long content wraps/truncates instead of overflowing the screen width.
    - `InspectionInterestCta.tsx` — S9a fake-door CTA rendered inside `ListingDetail.tsx` for active listings; opens an RNR `Sheet` with honest coming-soon copy, optional willingness-to-pay TMT input, auth-on-action for anonymous users, and success/error states; controlled by `ListingDetailScreen` so post-publish redirect can reopen it, and reopened after sign-in by a serializable `{ kind: "inspection", listingId }` pending action rather than a stored closure (#365)
    ...
    - `OwnerListingCard.tsx` — feed-card visual language adapted for owner management with status badge, Open and Edit actions; receives `brandName`/`modelName`/`cityName` props from `useFeedCatalogMaps` in parent; uses shared `buildVariantUrl` from `detail/buildVariantUrl.ts` for cover images (correctly routes `listing-videos` vs `listing-photos` buckets); cover image has `onError` fallback to placeholder. Same `shrink-0`/`min-w-0` overflow guard and one-line labels as `ListingCard`.
    - `DraftCard.tsx` — draft row/card with draft identity, progress, photo count, Resume CTA, and destructive Discard with `AlertDialog` confirmation; uses shared `buildVariantUrl` for cover images with `onError` fallback. Layout uses `shrink-0`/`min-w-0` so long draft identity text does not overflow.
...
**S9a post-publish inspection-interest prompt:** on successful publish, `sell.tsx` redirects to `/(public)/listings/{id}?inspectionInterest=1`; `ListingDetailScreen` reads the param and auto-opens the `InspectionInterestCta` sheet when the viewer is the listing owner. This exposes the seller-side fake-door without blocking publication.
...
The Home tab (`app/(tabs)/index.tsx`) also accepts `brandId` + `modelId` route params (sent by "See other Brand Model" on a closed Listing). When both are present it resets the filters, sets `brandId` and `modelIds: [modelId]`, applies them, and clears the params with `router.setParams` so the same link re-applies after the buyer edits filters.
...
`useListingModelCounts({ filters })` (`apps/mobile/src/api/listings/useListingModelCounts.ts`) provides the per-model popularity counts that power the model picker. It calls `GET /api/v1/listings/filter-options/models` with the current non-model filters (including required `brandId`), debounces by 300 ms, parses through `ListingsSchemas.ListingModelCountResponseSchema`, and is enabled only when `brandId` is present.
...
- Apply button label adapts: `"Apply"` when no active filters or when the live count is unavailable, `"Show results ({count})"` when the live count returns. The label is capped at one line to keep the fixed footer height stable.
