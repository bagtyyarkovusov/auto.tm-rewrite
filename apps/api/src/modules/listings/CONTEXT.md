...
- `apps/api/src/modules/listings/`:
  ...
  - `infrastructure/` — 11 adapters: `NullVinDecoder`, `NullContentClassifier`, `ChronologicalRankingAdapter` (full implementation per [ADR-0021](../../../../../docs/adr/0021-feed-ranking-port.md); applies `ListingFilterCriteria` including FX-aware price filtering via injected `ExchangeRatePort`), `EventEmitterListingEventPublisher` (S4 #86), `PrismaListingDraftRepository`, `PrismaListingRepository` (S4 #89), `PrismaListingMediaRepository` (S4 #91), `PrismaExchangeRateRepository` (S4 #89), `PrismaListingsReadRepository` (cross-context read surface), `MinioMediaStorageAdapter` (S4 #88), `SharpImageVariantGenerator` (S4 #91)
...
- **S5 (Search + listing detail)** — MLP filter sheet/search over the forward-defined `filters` param in `ListFeed`
- **S6 (Contact seller)** — listing-detail Message button becomes functional; simple contact threads consume listing summaries
- **S7 (Minimal admin + moderation)** — report-button UI + `banned` status activation; report submission is owned by `admin/` as `ContentReport` creation and may emit generic `ContentReportCreated` from `admin/`, not `ListingReported` from `listings/`. Listing report targets must be currently `active`; draft, archived, banned, soft-deleted, and other non-public listing targets are treated as not found, while visible non-reportable states such as `sold` return a validation error. Report submission does not change `Listing.status` or public visibility, pending reports do not block owner archive/delete flows while the listing remains owner-actionable, sellers cannot report their own listings, explicit admin ban is only `active -> banned`, unban is `banned -> active` with no `previousStatus`, banning blocks new contact/messages while existing conversation history remains readable, and user suspension blocks future listing mutations while listing visibility stays governed by listing status/admin bans. Banned listings are omitted from public feed/search/favorites and non-owner detail reads return not found; owner-scoped reads may show the listing with only a generic banned notice. Owner mutation routes cannot edit, attach/remove/reorder media, mark sold, archive, republish, or delete a banned listing until admin unban; owner delete/archive is not a workaround for admin ban. Unban does not clear favorites, fire saved-search notifications, or resolve/dismiss pending reports. Non-active ban attempts and non-banned unban attempts fail as moderation state conflicts without audit rows or state changes. Listing owners see no report metadata: no report count, report reason/details, reporter identity, report status, or admin notes; admin free-text reasons stay internal. `listings/` validates listing report targets through ports and owns the actual listing status transition through `ListingsAdminPort.banActiveListing` and `ListingsAdminPort.unbanBannedListing`; `admin/` orchestrates report resolution and audit around that port instead of writing listing rows directly. For S7 moderation writes, `ListingsAdminPort` must participate in the admin-owned transaction or expose a transaction-scoped adapter so listing status, report resolution, and audit write commit or roll back together; it must not open an independent write transaction for admin ban/unban. Successful ban/unban may emit internal `ListingBanned` / `ListingUnbanned` events only after the outer moderation transaction commits, but S7 ships no user-facing consumers, notifications, conversation system messages, or worker jobs; enforcement uses synchronous listing-status checks.
...
- **Post-MLP discovery bets** — saved-search match consumers, Favorite UX, availability hours for contact prefs
