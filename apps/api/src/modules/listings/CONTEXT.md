...
- `apps/api/src/modules/listings/`:
  ...
  - `application/` — 24 use-cases (S4 #88-#92, S8 #188, #216, #217): `CreateDraft`, `UpdateDraft`, `ValidateDraftStep`, `ListMyDrafts`, `DiscardDraft`, `PresignUpload`, `PublishListing`, `MarkSold`, `ArchiveListing`, `RepublishListing`, `DeleteListing`, `EditListing`, `AttachMedia`, `RemoveMedia`, `ReorderMedia`, `GetListingDetail`, `ListFeed`, `CountListings`, `CountListingModels`, `ListMyListings`, `GetExchangeRates`, `AddFavorite`, `RemoveFavorite`, `ListMyFavorites`
  - `infrastructure/` — 13 adapters: `NullVinDecoder`, `NullContentClassifier`, `ChronologicalRankingAdapter` (full implementation per [ADR-0021](../../../../../docs/adr/0021-feed-ranking-port.md); `rank()` returns `RankedListing { listing, photos }` items, reading each page's media in one batched include; applies `ListingFilterCriteria` including `modelIds` as `modelId IN (...)` and FX-aware price filtering via injected `ExchangeRatePort`; `modelCounts()` groups feed-eligible listings by `modelId` and returns `{ modelId, totalMatching }` sorted by `totalMatching DESC`, then `modelId ASC`), `EventEmitterListingEventPublisher` (S4 #86), `PrismaListingDraftRepository`, `PrismaListingRepository` (S4 #89), `PrismaListingMediaRepository` (S4 #91), `PrismaExchangeRateRepository` (S4 #89), `PrismaListingsReadRepository` (cross-context `ListingsReadPort` surface and in-context `ListingCardReadPort`; maps a page of rows with one media include and at most one exchange-rate read), `PrismaListingsAdminRepository` (S7 — transaction-scoped `ListingsAdminPort` adapter for admin ban/unban), `PrismaFavoriteRepository` (S8 #188), `MinioMediaStorageAdapter` (S4 #88; S11 signs direct PUTs against `MINIO_PUBLIC_URL`, performs server-side object operations against private `MINIO_ENDPOINT`, and infers `chat-attachments` from keys starting with `chat-attachments/`; bucket creation/policy setup is explicit infra tooling, not an API boot side effect), `SharpImageVariantGenerator` (S4 #91)
...
| `FeedRankingPort` | `FEED_RANKING_PORT` | `domain/ports/FeedRankingPort.ts` | Internal: `ListFeed`, `CountListings`, and `CountListingModels` use-cases; exposes `rank()` (returns `RankedListing { listing, photos: CardPhotos }`), `count()`, and `modelCounts()` |
| `ListingCardReadPort` | `LISTING_CARD_READ_PORT` | `domain/ports/ListingCardReadPort.ts` | Internal: `ListMyFavorites` (`getVisibleCards`) and `ListMyListings` (`getOwnerCards`). Bound with `useExisting` to `PrismaListingsReadRepository`. `ListingCard` = `ListingSummary` + `CardPhotos` + `mileageKm?`, `condition?`, `transmissionId?`, `engineTypeId?`, `contactPhone?`, `allowCalls` |
...
- `ListFeed` — public chronological feed; returns `ListingSummary` DTOs with `coverMediaKey`, `photoKeys` (up to 2, `sortOrder`), `photoCount`, `mileageKm?`, `condition?`, `transmissionId?`, `engineTypeId?` (omitted when unset), `displayPriceTmt`, and `sellerTrust.phoneVerified`. For a signed-in viewer it adds `isFavorited` from one `FavoriteRepository.favoritedListingIds` read per page; anonymous responses omit the field. A page makes a fixed number of SQL queries (listings, media, exchange rates, favorites) regardless of page size
...
| GET | `/api/v1/listings` | Public (auth optional) | `ListFeed` | Accepts `cursor`, `limit`, and MLP filters (`brandId`, `modelId`, `cityId`, `priceMin`, `priceMax`, `yearMin`, `yearMax`, `condition`). Includes `coverMediaKey` when listing has media, `photoKeys` + `photoCount` always, `mileageKm` / `condition` / `transmissionId` / `engineTypeId` when set, `sellerTrust.phoneVerified` for every persisted listing, and `isFavorited` only when the caller is authenticated |
