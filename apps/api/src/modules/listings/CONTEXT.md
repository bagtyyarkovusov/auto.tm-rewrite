...
- `apps/api/src/modules/listings/`:
  ...
  - `infrastructure/` — 13 adapters: `NullVinDecoder`, `NullContentClassifier`, `ChronologicalRankingAdapter` (full implementation per [ADR-0021](../../../../../docs/adr/0021-feed-ranking-port.md); applies `ListingFilterCriteria` including `modelIds` as `modelId IN (...)` and FX-aware price filtering via injected `ExchangeRatePort`; `modelCounts()` groups feed-eligible listings by `modelId` and returns `{ modelId, totalMatching }` sorted by `totalMatching DESC`, then `modelId ASC`), `EventEmitterListingEventPublisher` (S4 #86), `PrismaListingDraftRepository`, `PrismaListingRepository` (S4 #89), `PrismaListingMediaRepository` (S4 #91), `PrismaExchangeRateRepository` (S4 #89), `PrismaListingsReadRepository` (cross-context read surface), `PrismaListingsAdminRepository` (S7 — transaction-scoped `ListingsAdminPort` adapter for admin ban/unban), `PrismaFavoriteRepository` (S8 #188), `MinioMediaStorageAdapter` (S4 #88; S10 also ensures the `chat-attachments` bucket for conversation images and infers it from keys starting with `chat-attachments/`), `SharpImageVariantGenerator` (S4 #91)
