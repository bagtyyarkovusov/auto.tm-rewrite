...
Follow [CREATION.md](CREATION.md) and the canonical Sprint-start sequence in `docs/agents/sprint-transitions.md`:
...
5. Only after the complete issue set verifies, update allowed Sprint-plan metadata and the roadmap start together on a small docs branch and open the authorized PR.
...
- Never edit Sprint DoD or scope during creation. The only Sprint-plan edit at start is the pre-lock factual metadata transition described in `docs/agents/sprint-transitions.md`.
